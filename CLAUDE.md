...
- VaR crown SETTLED (2026-07-22, supersedes the 2026-07-20 provisional ruling): **FEATURED = har_rv_cal_t; BENCHMARK = garch_11.** Rationale, verbatim: on coverage, har_rv_cal_t is the best-calibrated model in the project (95% 4.94%, 2/7 rejects; 99% 1.26%, 1/7 — down from 4/7 under the normal quantile), and coverage is the primary VaR criterion because it is what the risk number claims to deliver. garch_11 wins the independence dimension outright (0/7 at both levels vs 2/7), and that is a real advantage, not a rounding error — breaches arriving in bursts are worse than breaches arriving evenly, since losses compound faster than capital replenishes. It is retained as the benchmark on every page and its independence advantage must be stated wherever the crown is stated. The genuine clustering is concentrated in JPM and XOM, so the honest framing is: har_rv_cal_t for accuracy of the risk level, garch_11 for timing behavior, and a production desk would plausibly run both. Condition checked before applying: the df-clamp addendum does NOT weaken har_rv_cal_t (6 high clamps of 553 estimated refits = 1.1%, zero low rejections, numbers unchanged by the low-clamp fix).
- Superseded (2026-07-20 provisional): **har_rv_cal featured**, garch_11 benchmark. Rationale: capital-efficiency dead heat (avg daily VaR 3.51% vs 3.53% at 95%), har_rv_cal's 95% miss is on the conservative side (4.48%), and it is least-bad at 99% (1.49%, 4/7 rejects vs garch's 1.85%, 6/7). ONE render check can still flip it: breach CLUSTERING on the VaR Breach Tracker — Kupiec tests frequency, not independence; if har_rv_cal's breaches visibly cluster in crisis windows while garch_11's spread out, the crown is revisited. Christoffersen independence test BUILT (stretch 1, 2026-07-21): garch_11 rejects independence 0/14 ticker×level tests, har_rv_cal 4/14 (3 genuine clustering: JPM both levels, XOM 99%). Evidence leans toward garch_11 but is NOT decisive — LR_cc splits by level (garch better at 95%, har_rv_cal better at 99%). Labelling deliberately unchanged.
- Student-t VaR BUILT (stretch 2, 2026-07-22): variance-matched t quantiles, df by MLE on standardized residuals, walk-forward monthly, stored as `<model>_t` coverage rows with median df. Results: 99% under-coverage narrows for every model (garch_11 1.85%→1.44%, 6/7→3/7 Kupiec rejects; har_rv_cal 1.49%→1.26%, 4/7→**1/7**); 95% coverage WORSENS (7.14%→7.93% avg) because a variance-matched t moves mass from shoulders to tails — its 95% multiplier is below the normal z. Neither distribution dominates at both levels; **har_rv_cal_t is the best-calibrated combination in the project** (95%: 4.94%, 2/7; 99%: 1.26%, 1/7). Independence is unchanged by the quantile swap (garch 0/7 both, har_rv_cal 2/7 both), so stretch 2 does NOT move the crown's independence evidence.
- MLflow tracking BUILT (stretch 3, 2026-07-28): optional extra `tracking`, local SQLite store (`mlflow.db`) + `mlruns/` artifacts, no server, no cloud. Uses **mlflow-skinny** deliberately — the full `mlflow` package pins pandas<3 and would fire the standing tripwire; the UI runs from an isolated `uvx` env instead. Degrades to a no-op when absent/disabled/failing (tests cover all three); ~8 s per run, <2% of the nightly job. pandas stayed at 3.0.3 — the tripwire did NOT need to fire.
- KNOWN DECAY PATTERN (recorded 2026-07-28, third instance): hand-written prose that *references* a ruling goes stale after the review that certified it, while generated blocks stay current because they regenerate. Instances so far: (1) the sliding backfill window, still described as anchored-by-intent after the window had eroded replayability; (2) the independence verdicts computed over 16 variants above a table rendering 8; (3) the README Dashboard section still naming har_rv_cal featured with the eyeball-clustering flip procedure, five weeks after both were superseded — `docs/powerbi_spec.md` had it right, so the *decision* was recorded correctly and only the prose restating it decayed. Practical rule: when a ruling changes, grep for every prose restatement of it, not just the section being edited. A cheap docs test asserting the featured-model string is consistent across README / CLAUDE.md / powerbi_spec.md was proposed 2026-07-28 and NOT built pending Aakrisht's approval.
- Git-history note (2026-07-28, for anyone reading the commit graph): PR #4 (Stretch 3, MLflow) was opened **stacked on `feat/stretch-2-student-t`**, not on main. Merging #3 into main snapshotted that branch ~15 s before #4 landed on it, so commit `e2bf707` was briefly unreachable from main even though all three stretch PRs showed as merged. PR #5 (audit batch 2) merged the lineage back in. Nothing was rewritten or force-pushed; the gap is visible in the graph and is a stacked-PR base-branch artefact, not a lost commit.
- MIN_FIT_OBS stays at 100 (ruled 2026-07-23; do not re-litigate). Raising it to ~250 would zero the t-df clamp count, since every clamp is a small-sample artefact — declined because a measured, diagnosed, handled failure is a stronger artefact than a silently avoided one; it would push the first ~12 months onto the normal quantile, trading one honest limitation for another while invalidating published numbers; and 100 is a defensible minimum for a two-parameter fit. The finding IS that df is hard to identify from short samples.
- CROWN PROCEDURE REVISED (2026-07-21, supersedes the eyeball-clustering flip condition). Aakrisht's original condition — settle the crown by visually judging breach clustering on the rendered VaR page — was superseded by the measurement built to serve it, which showed the procedure UNDERDETERMINED: (a) ~half of all independence rejections are ANTI-clustering (too-regular spacing rejects because independence implies random arrival, but only bunching is a risk failure, and an eyeball reads over-regular spacing as healthy), so rejection ≠ clustering; (b) the genuine clustering is TICKER-SPECIFIC (JPM, XOM only), so no single rendered ticker could have settled it. The crown is now decided on: (a) direction-split Christoffersen evidence, counting genuine clustering only and explicitly setting anti-clustering rejections aside; (b) Kupiec coverage; (c) capital cost — with the rendered breach page as illustration, not evidence. Crown remains provisional until after stretch 2 (Student-t may move both coverage and independence).
